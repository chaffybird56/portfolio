...
function Thumb({ src, alt }) {
  if (src?.endsWith(".mp4")) return <video className="card-thumb" src={src} autoPlay muted loop playsInline />;
  return <Image src={src} alt={alt} width={1200} height={675} className="card-thumb" />;
}

function Octocat() {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" className="opacity-80" aria-hidden="true" fill="currentColor">
